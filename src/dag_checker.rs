...
pub(crate) fn find_source_nodes(dag: &Dag) -> Vec<NodeId> {
    ...
    for node in dag.graph_nodes.nodes() {
        let incoming_nodes: Vec<NodeId> = dag.graph_nodes.neighbors_directed(node, Direction::Incoming).collect();
...
pub(crate) fn find_sink_nodes(dag: &Dag) -> Vec<NodeId> {
    ...
    let mut result = vec![];
    for node in dag.graph_nodes.nodes() {
        let outgoing_nodes: Vec<NodeId> = dag.graph_nodes.neighbors_directed(node, Direction::Outgoing).collect();
